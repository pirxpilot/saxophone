...
class Saxophone extends Writable {
  ...
  constructor() {
    ...
    this._decoder = new TextDecoder(state.defaultEncoding);
  ...
  _parseChunk(input, callback) {
    ...
    while (chunkPos < end) {
      if (input[chunkPos] !== '<') {
        ...
        this.emit(
          Node.text, { contents: input.slice(chunkPos, nextTag) }
        );
      ...
      if (nextChar === '!') {
        ...
        if (
          ...
          this.emit(
            Node.cdata, { contents: input.slice(chunkPos, cdataClose) }
          );
...
'${input.slice(chunkPos - 4)}'`
          ...
          this.emit(
            Node.comment, { contents: input.slice(chunkPos, commentClose) }
          );
      ...
      if (nextChar === '?') {
        ...
        this.emit(
          Node.processingInstruction, { contents: input.slice(chunkPos, piClose) }
        );
      ...
      if (this._waiting !== null) {
        switch (this._waiting.token) {
          case Node.text:
            ...
            this.emit(
              'text', { contents: this._waiting.data }
            );
            break;
          case Node.cdata:
